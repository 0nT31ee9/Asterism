...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    odom_sub_ = nh.subscribe("/ground_truth/state", 1, &EGOReplanFSM::odometryCallback, this);
    depth_goal_sub = nh.subscribe("/depth_goal_pose", 1, &EGOReplanFSM::Depth_goal, this);
  ...
  std::vector<Eigen::Vector3d> EGOReplanFSM::generateCatmullRomSpline(const std::vector<Eigen::Vector3d>& points, int subdivisions) {
      std::vector<Eigen::Vector3d> splinePoints;

      if (points.size() < 4) {
          std::cerr << "Need at least 4 points to create a spline." << std::endl;
          return splinePoints;
      }
      ...
      for (size_t i = 0; i < points.size() - 3; i++) {
          ...
          for (int j = 0; j <= subdivisions; j++) {
              ...
              Eigen::Vector3d point = b0 * p0 + b1 * p1 + b2 * p2 + b3 * p3;
  ...
  std::vector<std::vector<Eigen::Vector3d>> EGOReplanFSM::generateSplinesForAllSets(const std::vector<std::vector<Eigen::Vector3d>>& pointSets, int subdivisions) {
      std::vector<std::vector<Eigen::Vector3d>> allSplines;
      for (const auto& points : pointSets) {
          allSplines.push_back(generateCatmullRomSpline(points, subdivisions));
      }
      return allSplines;
  }

  // std::vector<std::vector<Eigen::Vector3d>> EGOReplanFSM::GenerateBezierCurve(const std::vector<Eigen::Vector3d>& controlPoints, int numberOfPoints) {
  //     auto interpolate = [](const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double t) {
  //         return (1 - t) * p1 + t * p2;
  //     };

  //     std::vector<std::vector<Eigen::Vector3d>> result;
  //     for (int i = 0; i < numberOfPoints; ++i) {
  //         double t = i / (numberOfPoints - 1.0);
  //         std::vector<Eigen::Vector3d> points = controlPoints;

  //         while (points.size() > 1) {
  //             std::vector<Eigen::Vector3d> newPoints;
  //             for (size_t j = 0; j < points.size() - 1; j++) {
  //                 newPoints.push_back(interpolate(points[j], points[j + 1], t));
  //             }
  //             points = newPoints;
  //         }
  ...
  std::vector<std::vector<Eigen::Vector3d>> EGOReplanFSM::GenerateBezierCurves(const std::vector<std::vector<Eigen::Vector3d>>& controlPointsSets, int numberOfPoints) {
    auto interpolate = [](const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double t) {
        return (1 - t) * p1 + t * p2;
    };
    ...
    for (const auto& controlPoints : controlPointsSets) {
        std::vector<Eigen::Vector3d> curve;

        for (int i = 0; i < numberOfPoints; ++i) {
            double t = static_cast<double>(i) / (numberOfPoints - 1);
            std::vector<Eigen::Vector3d> tempPoints = controlPoints;

            while (tempPoints.size() > 1) {
                std::vector<Eigen::Vector3d> newPoints;
                for (size_t j = 0; j < tempPoints.size() - 1; ++j) {
                    newPoints.push_back(interpolate(tempPoints[j], tempPoints[j + 1], t));
                }
                tempPoints = newPoints;
            }

            if (!tempPoints.empty()) {
                curve.push_back(tempPoints.front());
            }
        }
  ...
  {
    odom_pos_(0) = msg->pose.pose.position.x - 1.5;
    odom_pos_(1) = msg->pose.pose.position.y - 1.5;
    odom_pos_(2) = msg->pose.pose.position.z - 0.05;
    ...
    std::cout<< "odom_pos_(0)" << "  " <<odom_pos_(0) << odom_pos_(1) << "  " << odom_pos_(2) << std::endl;
    a_star_pathes =  planner_manager_->S_A(0.1, odom_pos_, Eigen::Vector3d(3, 0.4, 1.5));

    // a_star_pathes =  planner_manager_->S_A(0.1, odom_pos_, goal_pose);
    // double distance = (goal_pose - odom_pos_).norm();
    double distance = (goal_pose - Eigen::Vector3d(3, 0.4, 1.5)).norm();


    // std::cout << goal_pose. << endl;

    std::cout << "distance" << distance << endl;
    // std::cout << "distance"  << endl;


    // std::vector<std::vector<Eigen::Vector3d>> allSplines = generateSplinesForAllSets(a_star_pathes, 10);

    std::vector<std::vector<Eigen::Vector3d>> bezierCurve = GenerateBezierCurves(a_star_pathes, distance*5);
    visualization_->displayAStarList(bezierCurve, 0);

  }

  void EGOReplanFSM::Depth_goal(const ego_planner::goalPtr &msg)
    {
        goal_pose[0] = msg->x;
        goal_pose[1] = msg->y;
        goal_pose[2] = msg->z;

        std::cout << "goal_pose" << goal_pose[0] << "  "
        << goal_pose[1] << "  "
