...
void Path::DepthCallback(const sensor_msgs::Image::ConstPtr& msg){
    ...
    // uint16_t median_depth_less_than_7 = filtered_depth_values[filtered_depth_values.size() / 2];
        ...
        if(desire_label.empty() != 0 && is_detect == true){
            int32_t label_center_x;
            int32_t label_center_y;

            int radius = (std::min((desire_label[2] - desire_label[0]), (desire_label[3] - desire_label[1])) / 2);
            if (radius < 320 && radius > 0){
                radius = radius/2;
                // std::cout << "radius " << radius << std::endl;
                // 分析圆环边缘的深度值
                std::vector<uint16_t> depth_values;
                label_center_x = (desire_label[2] + desire_label[0])/2;
                label_center_y = (desire_label[1] + desire_label[3])/2;
                std::cout << "label_center_x" << "  "<< label_center_x << "  " << label_center_y << std::endl;
                for (double angle = 0; angle < 360; angle += 1.0) {
                    int x = label_center_x + radius * cos(angle * M_PI / 180.0);
                    int y = label_center_y + radius * sin(angle * M_PI / 180.0);
                    depth_values.push_back(cv_ptr->image.at<uint16_t>(y, x));
            }
...
// uint16_t median_depth_less_than_7 = filtered_depth_values[filtered_depth_values.size() / 2];
                ...
                double Y = (c_x - label_center_x) * X / f_x ;
                double Z = (c_y - label_center_y) * X / f_y;

                X = X + vs_current_x;
                Y = Y + vs_current_y;
                Z = Z + vs_current_z;

                std::cout << "Object Position: label = " << X << ", Y = " << Y << ", Z = " << Z << std::endl;
...
void Path::plan_odomCallback(const nav_msgs::Odometry::ConstPtr& msg){
    ...
    if(std::abs(msg->twist.twist.linear.x) > 4 || std::abs(msg->twist.twist.linear.y) > 4 || std::abs(msg->twist.twist.linear.y)>4
    || std::abs(vs_current_x - position.x) >2|| std::abs(vs_current_y - position.y) >2 || std::abs(vs_current_y - position.y) >2 ){
        setMode("AUTO.LAND");
        std::cout << "88888888888---------------wanging -----88888888888888" << std::endl;
    }
    ...
    _diss_x = std::abs(PlanTarget.position.x - vs_current_x);
    _diss_y = std::abs(PlanTarget.position.y - vs_current_y);
    _diss_z = std::abs(PlanTarget.position.z - vs_current_z);
...
void Path::localCallback(const geometry_msgs::PoseStamped::ConstPtr& msg){
    ...
    diss_x = std::abs(PositionTarget.position.x - current_x);
    diss_y = std::abs(PositionTarget.position.y - current_y);
    diss_z = std::abs(PositionTarget.position.z - current_z);
    ...
    std::cout << "diss_z:" << diss_z << std::endl;
...
void Path::a_star_listCallback(const visualization_msgs::Marker::ConstPtr& msg){
    ...
    if (!msg->points.empty()){
        if (msg->points.size() > 5){
            ...
            // PlanTarget.yaw = yaw;
            // PlanTarget.position.z = first_pose.pose.position.z;
            PlanTarget.coordinate_frame=PlanTarget.FRAME_LOCAL_NED;
            PlanTarget.type_mask=PlanTarget.IGNORE_VX|
                                PlanTarget.IGNORE_VY|
                                PlanTarget.IGNORE_VZ|
                                PlanTarget.IGNORE_AFX|
                                PlanTarget.IGNORE_AFY|
                                PlanTarget.IGNORE_AFZ|
                                PlanTarget.IGNORE_YAW|
                                PlanTarget.IGNORE_YAW_RATE;
            // std::cout << "--------------------hoving1----------------------------" <<std::endl;
            ...
            PlanTarget.type_mask=PlanTarget.IGNORE_VX|
                                ...
                                PlanTarget.IGNORE_YAW|
                                PlanTarget.IGNORE_YAW_RATE;
            // std::cout << "--------------------hoving2----------------------------" <<std::endl;

        }      
    }
    else{

            PlanTarget.velocity.z = 0;
            PlanTarget.velocity.x = 0;
            PlanTarget.velocity.y = 0;

            // double yaw = tf::getYaw(first_pose.pose.orientation);
            // PlanTarget.yaw = yaw;
            // PlanTarget.position.z = first_pose.pose.position.z;
            PlanTarget.coordinate_frame=PlanTarget.FRAME_LOCAL_NED;
            PlanTarget.type_mask= PlanTarget.IGNORE_PX|
                                 PlanTarget.IGNORE_PY|
                                 PlanTarget.IGNORE_PZ|
                                PlanTarget.IGNORE_AFX|
                                PlanTarget.IGNORE_AFY|
                                PlanTarget.IGNORE_AFZ|
                                PlanTarget.IGNORE_YAW|
                                PlanTarget.IGNORE_YAW_RATE;
            std::cout << "--------------------hoving----------------------------" <<std::endl;
...
void Path::_Fly1(){
             PositionTarget.velocity.z = 0.5;
            PositionTarget.velocity.x = 0;
            PositionTarget.velocity.y = 0;
            PositionTarget.position.z=3.5;
            // double yaw = tf::getYaw(first_pose.pose.orientation);
            // PlanTarget.yaw = yaw;
            // PlanTarget.position.z = first_pose.pose.position.z;
            PositionTarget.coordinate_frame=PositionTarget.FRAME_LOCAL_NED;
            PositionTarget.type_mask= PositionTarget.IGNORE_PX|
...
void Path::_Fly2(){
    PositionTarget.position.x=10.1;
    PositionTarget.position.y=0.5;
    PositionTarget.position.z=3.5;
    PositionTarget.velocity.z = 0.1;
    PositionTarget.velocity.x = 0.5;
    PositionTarget.velocity.y = 0.1;
    PositionTarget.coordinate_frame=PositionTarget.FRAME_LOCAL_NED;
    PositionTarget.type_mask=PositionTarget.IGNORE_AFX|PositionTarget.IGNORE_AFY|PositionTarget.IGNORE_AFZ|PositionTarget.IGNORE_YAW|PositionTarget.IGNORE_YAW_RATE;
}
    ...
    while(ros::ok()){
        ...
        switch(FlyState)
        {
            ...
            case Circle_2:
                ...
                if (line_strip.size() < 3 && elapsed_time >1)      
                {
                    FlyState = Circle_2_3;
                }
            ...
            case Circle_2_3:
                ...
                path_pub.publish(PositionTarget);
                FlyState = Circle_2_3;
                std::cout << "Circle_2_3" << std::endl;

                current_time = ros::Time::now().toSec();
                elapsed_time = current_time - start_time;

                if (diss_z < 0.1){
                    FlyState = Circle_2_2_3;
                }
            ...
            case Circle_2_2_3:
                ...
                path_pub.publish(PositionTarget);
                std::cout << "Circle_2_2_3" << std::endl;
                std::cout << distance_to_target<< std::endl;

                if (distance_to_target < 0.16 && diss_z < 0.05) 
                {
                    FlyState = Circle_3;
                }
                up = 0;
            ...
            case Circle_3:
                ...
                if (label_depth.x > 1){
                    std::cout << "deplabel_depthth" <<label_depth.x << "  "  << label_depth.y<< "  "<< label_depth.z<<std::endl;
                    depth_goal_pub.publish(label_depth);
                }
                fixed();
                path_pub.publish(PositionTarget);
                // path_pub.publish(PlanTarget);
