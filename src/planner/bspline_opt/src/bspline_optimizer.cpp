...
namespace ego_planner
{
  ...
  vector<vector<Eigen::Vector3d>> BsplineOptimizer::seven_search(const double step_size, Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
  {
    a_star_->AstarSearch(/*(in-out).norm()/10+0.05*/ 0.1, start_pt, end_pt);
    // a_star_->getPath();
    std::cout << "end_pt: " << end_pt <<std::endl;
    for (const auto& point : a_star_->getPath()){
